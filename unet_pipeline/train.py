  ...
  def run_training(self, model, train_loader, valid_loader, fold):
    ...
    for epoch in range(last_epoch + 1, self.num_epochs + 1): 
        ...
        if val_dice >= best_dice:
            print(f"{c_}Valid Score ImproSved ({best_dice:0.4f} ---> {val_dice:0.4f})")
            best_dice = val_dice
            best_jaccard = val_jaccard
            best_epoch = epoch
            best_model_wts = copy.deepcopy(model.state_dict())
            
            torch.save(model.state_dict(), self.CFG['PATH_weights'] + f"best_epoch-{fold:02d}.bin")
            print(f"Model Saved{sr_}")
            
        last_model_wts = copy.deepcopy(model.state_dict())

        torch.save(model.state_dict(), self.CFG['PATH_weights'] + f"last_epoch-{epoch:02d}.bin")
